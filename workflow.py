...
def import_transactions(csv_path: str) -> pd.DataFrame:
    """Läser in / förbereder transaktionsdata."""
    try:
        df = pd.read_csv(csv_path, dtype=str)
...
def clean_and_normalize(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """Rensar & normaliserar kunddata till customers, addresses, accounts."""
...
def create_transactions_table():
    """
    Skapar tabellen 'transactions' om den inte redan finns .
    """
    ddl = """
...
def write_to_postgres(df: pd.DataFrame, table_name: str, db_url: str):
    """Skriveer  DataFrame till Postgres."""
    if df.empty:
...
@flow
def load_data_flow():
    """Hela ETL‐flödet: import → rensa → skapa tabeller → skriv till databas."""
    ...
    # Skapa transactions-tabellen innan den fylls
